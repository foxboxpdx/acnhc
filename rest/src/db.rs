// DB stuff to keep lib relatively clean
extern crate diesel;
use diesel::prelude::*;
use super::models::*;
use super::schema;
...
impl User {
    ...
    fn read_by_username(conn: &SqliteConnection, uname: String) -> Option<User> {
        use schema::users::dsl::*;
        use schema::users;
        let u = users::table.filter(username.eq(uname.to_string())).get_result(conn);
        match u { Ok(x) => Some(x), Err(_) => None }
    }
}
