...
pub trait Item {
    fn create<'a>(conn: &PgConnection, name: &'a str) -> usize;
    fn load(conn: &PgConnection) -> Vec<Self> where Self: Sized;
    fn count(conn: &PgConnection) -> i64;
}
...
impl Item for Fossil {
    ...
    fn load(conn: &PgConnection) -> Vec<Fossil> {
...
impl Item for Recipe {
    ...
    fn load(conn: &PgConnection) -> Vec<Recipe> {
...
impl Item for Art {
    ...
    fn load(conn: &PgConnection) -> Vec<Art> {
        use schema::arts::dsl::*;
        arts
            .order(name.asc())
...
pub trait OwnedItem {
    fn create(conn: &PgConnection, u: i32, i: i32, e: i32) -> usize;
    fn load(conn: &PgConnection, uid: i32) -> Vec<Self> where Self: Sized;
    fn load_all(conn: &PgConnection) -> Vec<Self> where Self: Sized;
    fn count(conn: &PgConnection, uid: i32) -> i64;
    fn update(conn: &PgConnection, records: Vec<(i32, i32)>);
}
...
pub trait NewOwnedItem {
    fn batch_create(conn: &PgConnection, records: Vec<Self>) where Self: Sized;
}

impl NewOwnedItem for NewOwnedfossil {
    fn batch_create(conn: &PgConnection, records: Vec<NewOwnedfossil>) {
        use schema::ownedfossils;
...
impl NewOwnedItem for NewOwnedrecipe {
    fn batch_create(conn: &PgConnection, records: Vec<NewOwnedrecipe>) {
        use schema::ownedrecipes;
...
impl NewOwnedItem for NewOwnedart {
    fn batch_create(conn: &PgConnection, records: Vec<NewOwnedart>) {
        use schema::ownedarts;
...
pub fn get_uid_from_uname(conn: &PgConnection, name: &str) -> i32 {
...
pub fn get_user_from_uname(conn: &PgConnection, name: &str) -> User {
    use schema::users::dsl::*;
...
pub fn set_user_alias(conn: &PgConnection, uid: i32, a: &str) -> bool {
    use schema::users::dsl::*;
...
pub fn create_user<'a>(conn: &PgConnection, n: &'a str, a: &'a str) -> usize {
    use schema::users;
...
pub fn create_recipetype<'a>(conn: &PgConnection, name: &'a str) -> usize {
...
pub fn recipes_by_category(conn: &PgConnection) -> BTreeMap<String, Vec<Recipe>> {
